...
The cleaned data set `census_data` is already post-stratified. The column variable `n` represents the number of respondents within the same group of state, gender, age, race, and employment status.
...
We are interested in predicting the popular vote outcome of the 2020 American federal election (include citation). To do this we will build *two* binary logistic regression models based on the Democracy Fund + UCLA Nationscape data (the `survey_data`), and apply the fitted models to the ACS data set (the `census_data`) to make predictions for proportion of voters who advocate Donald Trump/Biden, in another words, the probability of Trump/Biden wining the election. We will also utilize a post-stratification technique to account for any under-represented groups in the voting population. In the following sub-sections I will describe the model specifics and the post-stratification calculation.
...
4. Student;
5. Other.
* Homemaker, retired people, and disabled individuals are categorized as "not in labor force".
...
The logistic regression models we are using is:

+ Donald Trump:
$$ y_{vote\_trump} = \beta_0 + \beta_1 x_{gender} + \beta_2 x_{age} + \beta_3 x_{race\_ethnicity} + \beta_4 x_{employment} + \beta_5 x_{state\_name} + \epsilon$$
...
+ Joe Biden:
$$ y_{vote\_biden} = \beta_0 + \beta_1 x_{gender} + \beta_2 x_{age} + \beta_3 x_{race\_ethnicity} + \beta_4 x_{employment} + \beta_5 x_{state\_name} + \epsilon$$
...
kbl(broom::tidy(model_logit_trump))
```
...
kbl(broom::tidy(model_logit_biden))
```
